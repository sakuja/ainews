...
function layout({ title, description, base, body, path, ogType = "website" }) {
...
export function renderList({ articles, all, base, heading, page, totalPages, pageHref, path }) {
  const pager =
    totalPages > 1
  ...
  const body = `${heading ? `<h1 class="list-heading">${esc(heading)}</h1>` : ""}
    ...
    ${pager}`;
  return layout({ title: heading, base, body, path }).replace("__SIDEBAR__", sidebar(all, base));
}
...
export function renderArticle(a, all) {
  ...
  return layout({
    ...
    base,
    body,
    path: `archives/${a.id}.html`,
    ogType: "article",
  }).replace("__SIDEBAR__", sidebar(all, base));
}

/** pages: { path, lastmod, priority } の配列 */
export function renderSitemap(pages) {
  const urls = pages
    .map(
      (p) =>
        `<url><loc>${esc(absUrl(p.path))}</loc>${p.lastmod ? `<lastmod>${new Date(p.lastmod).toISOString().slice(0, 10)}</lastmod>` : ""}${p.priority ? `<priority>${p.priority}</priority>` : ""}</url>`,
    )
    .join("\n");
